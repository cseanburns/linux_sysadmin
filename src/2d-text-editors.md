...
1. **Understand the role of text editors**: Recognize the importance of text editors in the command-line environment for saving commands, writing scripts, and editing configuration files.
2. **Differentiate between text editors**: Identify key differences between line editors like `ed` and visual editors like `vim`, `nano`, and `tilde`.
3. **Operate a line editor**: Use basic commands in `ed`, such as addressing lines, manipulating text, and editing files.
4. **Explore the functionality of `vim`**: Understand the basic modal operation of `vim`, including command mode, insert mode, and how to integrate shell commands within the editor.
5. **Utilize beginner-friendly editors**: Comfortably use `nano`, `tilde`, and `edit` for straightforward text editing tasks, taking advantage of their user-friendly key bindings and interfaces.
6. **Appreciate historical context**: Recognize the historical significance of `ed` and `vim` and how their development has influenced modern computing practices.
...
In this lesson, we'll learn about several text editors: `ed`, `vim`, `nano`, `tilde`, and `edit`.
We cover `ed` primarily for its historical importance, and its descendant, `vim`, which is a powerful editor that is widely used today, but these editors have high learning curves.
`emacs` is another common text editor that can be used on the command line, but it also has a learning curve, and I am not covering it here.
For simplicity, I will encourage you to use `nano`, `tilde`, or `edit`, but if you continue to use the command line, you should learn more advanced editors like `vim` or `emacs`.
...
It is the same basic interface you are using now when you use your terminal applications, which are virtualized versions of those old teletypewriters.
...
Using `ed`, we can remove the header line of the **operating-systems.csv** file by specifying the line number (`1`), and issuing the delete command (`d`), just like in `sed`.
...
Let's start fresh.
In the following session, I'll start `ed`, enter append mode `a`, write a short letter, exit append mode `.`,
...
Generally, we started with `ed`, which influenced the creation of [vi][vi], which led eventually to `vim`, aka, **Vi IMproved**.
...
I hope you find this really interesting.
Feel free to practice and play on the command line, as well as use tools like ed, the standard editor.
...
There isn't any difference on an empty file, but on a file that has text, `i` will start **insert** mode before the cursor position, and `a` will **insert** mode after the cursor position.
...
I press **Esc** to enter **command mode** (if I'm not already in it), enter a colon, type `r` for the *read into buffer* command,
...
`vim` has had such a powerful impact on software development that programmers have built these keystrokes into applications like Gmail, Facebook, and more.
...
However, if you want to use a more modern modeless editor, then I suggest `tilde` or `edit`, both of which I've installed on the Linux server.
...
Or you can launch it with a file name as an argument:

```
tilde [file_name]
```

> **Note:** `[file_name]` is just a placeholder for the name of a file you supply.
> E.g., if you want to open a file named `paper.txt`, then use `tilde paper.txt`
...
The [edit][ms_edit] text editor is an open source editor from Microsoft that's inspired by the original MS-DOS editor from the early 1990s.

```
edit [file_name]
```

The `edit` editor uses standard keyboard combinations for opening (`ctrl-o`) and saving (`ctrl-s`) files;
cutting (`ctrl-x`), pasting (`ctrl-v`), copying (`ctrl-c`) text; for undoing prior edits (`ctrl-z`); and for quitting (`ctrl-q`) the program.
You can also use the mouse with the `edit` text editor.

> For those interested in how these keyboard combinations came to become standard, see [IBM Commun User Access][ibm_cua].
