...
To build on our prior example, we can add ``>>`` to send the output to a new file called **output.txt**:

```
sort file.txt | uniq >> output.txt
```

We have available more powerful utilities and programs to process, manipulate, and analyze text files.
...
Basically, ``grep`` "prints lines that match patterns" (see ``man grep``).
In other words, it's search, and it's super powerful.
...
To do so, we use what's called a **regular expression**, or **regex** for short.
...
To use ``grep`` to remove the first line of a file, we can invert our search to select all lines not matching "OS" at the start of a line.
...
Let's see how to use ``sed`` to print line numbers instead of using the ``nl`` command.
To do so, we use the equal sign ``=`` to identify line numbers.
Note that it places the line numbers just above each line:
...
As long as we don't use the `-i` operator, we don't change the actual file.
...
then pipe ``|`` the output through the ``sort`` command to sort the numbers in numeric order:
...
In the next example, I print only those lines where column three is greater than 1990, and the line has the string "BSD" in it:
...
If you're interested in learning more, then check out some of these tutorials:
...
parts of a file and then pipe those parts through other commands or redirect output to files.
...
Like the utilities that we've discussed in prior lectures, the power of programs like these is that their on hand and easy to use as **one-liners**.
